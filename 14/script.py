...
class Map:
    ...
    def __init__(self, width, height, robots):
        ...
        for robot in robots:
            posX, posY = robot["pos"]
            velX, velY = robot["vel"]
            valAtPos = self.map[posY][posX]

            if valAtPos == self.Empty:
                self.map[posY][posX] = []
            
            self.map[posY][posX].append((velX, velY))
    ...
    def countRobotsInQuadrants(self):
        xMid = round(self.height / 2) - 1
        yMid = round(self.width / 2) - 1
        # xMidMin = round(self.height / 2) - 1
        # xMidMax = xMidMin
        # yMidMin = round(self.width / 2) - 1
        # yMidMax = yMidMin
        ...
        for x in range(len(self.map)):
            for y in range(len(self.map[0])):
                if x == xMid or y == yMid:
                    continue
                # if x >= xMidMin and x <= xMidMax or y >= yMidMin and y <= yMidMax: 
                #     continue
...
for line in input:
    ...
    vel = vel.split("=")[1]
    velX,velY = [int(i) for i in vel.split(",")]
